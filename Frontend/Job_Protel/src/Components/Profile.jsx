      ...
      <div className="max-w-4xl mx-auto bg-white border border-gray-200 rounded-2xl my-8 p-8 shadow-lg">
        <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-6">
          ...
          <div className="flex items-center gap-6">
            <Avatar className="h-28 w-28 shadow-md border-2 border-black">
              <AvatarImage
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">
                {authUser?.profile?.bio || "Add your bio here"}
              </p>
            </div>
          </div>
          {/* Edit Button */}
          ...
          <div className="flex flex-wrap gap-2">
            {authUser?.profile?.skills?.length ? (
              authUser.profile.skills.map((skill, index) => (
                <Badge
                  key={index}
                  className="bg-[#6A38C2] text-white font-semibold px-3 py-1 rounded-full shadow-sm"
                >
                  {skill}
                </Badge>
              ))
        ...
        <div className="mt-6">
          ...
          {authUser?.profile?.resume ? (
            ...
            >
              {authUser.profile.resumeOriginalName || "Resume"}
            </a>
          ) : (
